...
export default {
	...
	computed: {
		...
		playerGoals() {
			return !_.isEmpty(this.match) && !_.isEmpty(this.player)
				? this.match.players[this.player.id].goals
				: 0
		},
		playerAssists() {
			return !_.isEmpty(this.match) && !_.isEmpty(this.player)
				? this.match.players[this.player.id].assists
				: 0
		},
		result() {
			return !_.isEmpty(this.match) && !_.isEmpty(this.player)
				? this.match.players[this.player.id].local == 'home'
